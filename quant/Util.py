...
def triNet(sample,short=5, long=15, freq='60min'):
    ...
    for i in range(1, len(sample)):
        ...
        temp = ms[ms.index.get_level_values(index).strftime(dayformate) == sample.index.get_level_values(dayindex)[i].strftime(dayformate)][:-1]
        tmp = ms[ms.index.get_level_values(index).strftime(dayformate) == sample.index.get_level_values(dayindex)[i - 1].strftime(dayformate)][-1:].single[0]
...
def triNetDay(sample,short=5, long=15):
    #week and day syntony together is not good 17
    #to get Week and 60 minutes syntony together
    #get week trend
    import quant.weekTrend as wt
    wstart = '2010-01-01'
    ...
    for i in range(1, len(sample)):
        ...
        # here use index to get value interested, here we take change of MACDBlock to get the short trend in week level
        direction = wd.loc[windex].change
        if(direction>0 and single[i]==1):
            sig.append(1)
        elif(direction<0 and single[i]==3):
            sig.append(3)
        else:
            sig.append(0)
...
def trail_plot(sample):
    """
    this is to virsilize the applied single
    """
    N = sample.shape[0]
    ind = np.arange(N)
    import quant.MACD as macd
    quotes = macd.MINcandlestruct(sample, dayindex, dayformate)

    def format_date(x, pos=None):
        thisind = np.clip(int(x + 0.5), 0, N - 1)
        return sample.index.get_level_values(dayindex)[thisind].strftime(dayformate)
...
def bt2():
    ...
    # endtime = str(cur.year) + '-' + str(cur.month) + '-' + str(cur.day)
    endtime = '2020-05-25'
    #cl = ['000977', '600745','000810','002889','002475','002241']
    cl50 = ['000977', '600745', '002889', '600340', '000895', '600019', '600028', '601857', '600585', '002415',
            '002475', '600031',
            '600276']
    # data = loadLocalData(codelist, '2019-01-01', endtime)
    data = loadLocalData(cl50, '2019-01-01', endtime)
